...
function doPost(e) {
  try {
    ...
    if (!secret || !folderId) return reply_({ success: false, error: "config" });
    var body = JSON.parse(e.postData.contents);
    if (!body || body.v !== "1" || ["upload", "read", "delete"].indexOf(body.op) < 0 ||
    ...
    var canonical = ["1", body.op || "", String(body.ts || ""), body.nonce || "",
