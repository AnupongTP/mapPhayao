...
function buildPointResponse({ latitude, longitude, row, soilSuitability }) {
  ...
  const found = Boolean(amphoe || tambon || basin || soil);
  const suitabilityStatus = soilSuitability.status === "EVALUATED"
    ? "PARTIALLY_EVALUATED"
    : "NOT_EVALUATED";
  ...
  return {
    ...
    soilSuitability,
    recommendedRiceVarieties: [],
    waterRegime: "UNKNOWN",
    suitability: {
      soilClass: soilSuitability.status === "EVALUATED"
        ? soilSuitability.class
        : null,
      waterClass: null,
